...
struct OnroadEvent @0xc4fa6047f024e718 {
  ...
  enum EventName @0x91f1992a1f77fb03 {
    ...
    # Renumbered 2026-08-15 during the xnor-dev resync merge - the original 99-103 slice
    # of this block (picked back in March 2026) collided with five entirely different
    # events upstream added since then (lateralManeuver@99, bigModelLoading@100,
    # bigModelReadyDEPRECATED@101, bigModelFailed@102, carNotReady@103). Only those 5 got
    # moved, to the first free numbers right after this enum's actual highest ordinal
    # (116, speedLimitPending below) - capnp enums require the full ordinal set to be a
    # gap-free sequential range, so 104-116 (never actually colliding with anything) had
    # to stay exactly where they were. Two earlier attempts at this fix both broke the
    # build ("Skipped ordinal"): first by moving the whole 99-116 block to 117-134 and
    # leaving 104-116 unused, then by anchoring the 5 relocated names off that same
    # now-stale 134 (135-139) instead of the real max once 104-116 were put back.
    lkasEnable @117;
    lkasDisable @118;
    silentLkasEnable @119;
    silentLkasDisable @120;
    silentDoorOpen @121;
...
struct SelfdriveState {
  ...
  # Renumbered from @13 to @14 during the xnor-dev resync merge (2026-08-15) - @13 now
  # collides with alertSound, which upstream renumbered to @13 in its own history since
  # our fork point; @0-@13 are all taken in this struct (0-7,9-13 in use, 8 taken by the
  # deprecated alertSound group below), so @14 is the first genuinely free field number.
  madsSP @14 :Custom.ModularAssistiveDrivingSystem;
